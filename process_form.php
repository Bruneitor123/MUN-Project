...
$paises_general = array_combine($paises_general_sinvalue, $paises_general_sinvalue);

$paises_general_valores = array(
  'Estados Unidos de América' => 0,
  'Federación de Rusia' => 0,
  'República Popular China' => 0,
  'Reino Unido de Gran Bretaña e Irlanda del Norte' => 0,
  'República Francesa' => 0,
  'Ucrania (Україна)' => 0,
  'Reino de Arabia Saudita' => 0,
  'Emirato Islámico de Afganistán' => 0,
  'Estados Unidos de Venezuela' => 0,
  'República de la India' => 0,
  'Confederación Suiza' => 0,
  'Estado de Israel' => 0,
  'República de Haití' => 0,
  'Estado del Japón' => 0,
  'Estados Unidos Mexicanos' => 0
);

$delegados_nacionales_sinvalue = array(
...
$delegados_nacionales = array_combine($delegados_nacionales_sinvalue, $delegados_nacionales_sinvalue);


$delegados_nacionales_valores = array(
  'Departamento de Oruro' => 0,
  'Departamento de Cochabamba' => 0,
  'Departamento de La Paz' => 0,
  'Departamento de Potosí' => 0,
  'Departamento de Chuquisaca' => 0,
  'Departamento de Tarija' => 0,
  'Departamento de Santa Cruz' => 0,
  'Departamento del Beni' => 0,
  'Departamento de Pando' => 0,
  'República de la India' => 0,
  'Confederación Suiza' => 0,
  'Estado de Israel' => 0,
  'República de Haití' => 0,
...
$paises_comision_paz_sinvalue = array(
  'Ucrania (Україна)',
  'Reino de Arabia Saudita',
  'Emirato Islámico de Afganistán',
  'Estados Unidos de Venezuela',
...
$paises_comision_paz = array_combine($paises_comision_paz_sinvalue, $paises_comision_paz_sinvalue);


$paises_comision_paz_valores = array(
  'Ucrania (Україна)' => 0,
  'Reino de Arabia Saudita' => 0,
  'Emirato Islámico de Afganistán' => 0,
  'Estados Unidos de Venezuela' => 0,
  ...
  'Estado del Japón' => 0,
  'Estados Unidos Mexicanos' => 0,
  'República Italiana' => 0,
  'República Federal de Alemania' => 0,
  'Países Bajos (Holanda)' => 0,
  'República de Corea' => 0,
  'República de Sudáfrica' => 0
);


  // prepare and execute query to get count of each option value
  $stmt = $conn->prepare("SELECT equipo_diplomatico, COUNT(1) FROM bassmun_registro WHERE foro = '$foro' GROUP BY equipo_diplomatico");
  $stmt->execute();
  $result = $stmt->get_result();

      if ($foro == 'Comité de Tierra y Territorio, Recursos Naturales y Hoja de la Coca (Foro Nacional)') {
          while ($row = $result->fetch_assoc()) {
              $option_value = $row['equipo_diplomatico'];
              $count = $row['COUNT(1)'];
              if (array_key_exists($option_value, $delegados_nacionales_valores)) {
                  $delegados_nacionales_valores[$option_value] = $count;
              }
          }
          foreach ($delegados_nacionales_valores as $option => $count) {
              if ($count >= 1) {
                unset($delegados_nacionales[$option]);
                $random_index = array_rand($delegados_nacionales);
                $valor_pais_random = $delegados_nacionales[$random_index];
              }
          }
          ...
          while ($row = $result->fetch_assoc()) {
              $option_value = $row['equipo_diplomatico'];
              $count = $row['COUNT(1)'];
              if (array_key_exists($option_value, $paises_comision_paz_valores)) {
                  $paises_comision_paz_valores[$option_value] = $count;
              }
          }
          foreach ($paises_comision_paz_valores as $option => $count) {
              if ($count >= 1) {
                unset($paises_comision_paz[$option]);
                $random_index = array_rand($paises_comision_paz);
                $valor_pais_random = $paises_comision_paz[$random_index];
              }
          }
      } else {
          while ($row = $result->fetch_assoc()) {
            $option_value = $row['equipo_diplomatico'];
            $count = $row['COUNT(1)'];
            if (array_key_exists($option_value, $paises_general_valores)) {
                $paises_general_valores[$option_value] = $count;
            }
        }
        foreach ($paises_general_valores as $option => $count) {
            if ($count >= 1) {
              unset($paises_general[$option]);
              $random_index = array_rand($paises_general);
              $valor_pais_random = $paises_general[$random_index];            
            }
        }
      }
...
if ($_FILES['comprobante']['error'] == UPLOAD_ERR_OK) {
  ...
  if (in_array($extension, $allowed_types) && $_FILES['comprobante']['size'] <= $max_size) {
    ...
    echo 'Error: Tipo de archivo inválido (Solo PDF) o archivo con tamaño muy grande (Hasta 1MB)';
  }
} else {
